...
async def check():
    ...
    if janvii:
        ...
        if payroll:
            print(f"Month: {payroll.get('month')}")
            print(f"Net: {payroll.get('netSalary')}")
            print(f"Deductions: {payroll.get('deductions')}")
            print(f"Remarks: {payroll.get('deductionRemarks')}")
        else:
            print("No payroll found for May")
