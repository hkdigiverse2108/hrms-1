...
export default function MyTasksPage() {
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const [tasksRes, wmTasksRes, entriesRes, otherWorkRes, projectsRes, clientsRes] = await Promise.all([
  ...
  const allConsolidatedTasks = useMemo(() => {
    ...
    return consolidated
  }, [tasks, wmTasks, entries, otherWork, projects, clients, currentUser])
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Track and manage your tasks across all departments in one unified dashboard.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-500">Filter by Date:</span>
          <Popover>
            ...
            <PopoverContent className="w-auto p-0 border border-slate-200 bg-white shadow-lg rounded-xl" align="end">
              <DayCalendar
                initialFocus
                mode="range"
          ...
          {activeDateRange?.from && (
            <Button
              ...
              className="h-9 px-2.5 text-xs font-bold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100/80 rounded-lg border border-red-200 transition-colors"
            >
                                  ...
                                  <tr key={task.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                      ...
                                      <div className="text-[10px] text-slate-400 mt-0.5">
                                        Assigned by: {task.originalTask?.assignerName || 'TL Graphics'} {task.originalTask?.assigneeName ? `Assigned to: ${task.originalTask.assigneeName}` : `Assigned to: ${currentUser?.name}`}
                                      </div>
                                    </td>
