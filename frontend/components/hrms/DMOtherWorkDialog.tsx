...
export function DMOtherWorkDialog() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const computedTaskType = "dm-other-work";
      ...
      if (res.ok) {
        toast.success("Other Work assigned successfully!");
        setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-slate-800">Assign Other Work (Digital Marketing)</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            <Label className="text-slate-700">Assign To (Digital Marketing) <span className="text-rose-500">*</span></Label>
              ...
              <PopoverContent className="w-[--radix-popover-trigger-width] p-0" align="start">
                ...
                <div className="max-h-56 overflow-y-auto p-1">
                  {employees.filter((e: any) => {
                    ...
                    return name.includes(term) && (e.department?.toLowerCase().includes("digital marketing"));
                  }).map((emp: any) => (
                    <div 
                      key={emp.id}
