...
export const CELEBRATION_PRESETS: Record<string, { label: string; description: string }> = {
  poppers: {
    label: "Party Poppers",
    description: "Diagonal confetti cannons firing from bottom corners"
  },
  rain: {
    label: "Sparkles Rain",
    description: "Full-screen cascading glitter and starburst rain"
  },
  fireworks: {
    label: "Fireworks Burst",
    description: "Explosive radial starburst expanding from screen center"
  },
  stars: {
    label: "Floating Stars",
    description: "Gentle floating golden stars & glowing diamond aura"
  },
  none: {
    label: "None",
    description: "No celebration animation on login"
  }
};
...
export function SparklesCelebration({ 
  ...
  if (!visible || activeEffect === "none" || particles.length === 0) return null;
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[9999] overflow-hidden">
      ...
      {/* 3. Floating Stars (Gentle Rising Golden Stars) */}
      {activeEffect === "stars" && (
        <div className="absolute inset-0 z-40 overflow-hidden">
          {particles.map((p) => (
            <div
              key={p.id}
              className="absolute flex items-center justify-center opacity-0 animate-sparkle-stars"
              style={{
                left: `${p.left}%`,
                bottom: `-20px`,
                animationDelay: `${p.delay}s`,
